...
### PBS -l select=11:ncpus=40:model=cas_ait
#PBS -q long
...
RUNDIR=/nobackupp28/gkoban/Realtime/SWMF/run_realtime
#
##### Remove stop files
...
do
	...
	do
	    ...
	    CME_CUTOFF="2025-11-09 07:24"
	    # Guard: stop BEFORE we touch anything if the next cycle would pass the CME time
	    python3 $SWMF_dir/stop_before_cme.py "$RUNDIR/SC/ENDMAGNETOGRAMTIME.in" "$CME_CUTOFF"
	    rc=$?
	    if [ $rc -eq 1 ]; then
		    echo "Reached CME cutoff ($CME_CUTOFF). Exiting without modifying files."
		    exit 0
	    elif [ $rc -ne 0 ]; then
		    echo "Time guard failed (rc=$rc). Safer to stop."
		    exit 1
	    fi

	    python3 $SWMF_dir/update_PF.py
	    python3 $SWMF_dir/update_magnetogram_from_timejson.py
		...
		# sleep 300
		#instead of sleeping 5 minutes, update time
		python3 $SWMF_dir/update_time_5min.py
		python3 $SWMF_dir/update_magnetogram_from_timejson.py
		python3  $SWMF_dir/get_magnetogram_pleiades.py
		cd $RUNDIR/SC
		tar -xzvf submission.tgz
	    ...
	    ./Restart.pl -v
	    python3 $SWMF_dir/update_time.py
	done
    done
done
./PostProc.pl -M -cat RESULTS_`date +%y%m%d_%H%M`
